...
/// assert!(
///     vs.iter().filter_map(|v| v.inner.as_ref()).all(|inner_ref| inner_ref.known_value.is_none())
/// );
///
///
///
...
/// assert!(
///     vs.iter().filter_map(|v| v.inner.as_ref()).all(|inner_ref| inner_ref.known_value.is_some())
/// );
///
/// # anyhow::Ok(())
/// # }).unwrap();
